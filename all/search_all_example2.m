...
method = 'AStar'; % Options: 'AStar', 'BestFS', 'Uniform', 'DFS', 'BFS'
heuristicTable = containers.Map({'Arad', 'Bucharest', 'Craiova', 'Drobeta', 'Eforie', 'Fagaras', 'Giurgiu', 'Hirsova', 'Iasi', 'Lugoj','Mehadia','Neamt','Oradea','Pitesti','R.Vilcea','Sibiu','Timisoara','Urziceni','Vaslui','Zerind'}, ...
    [366, 0, 160, 242, 161, 176, 77, 151, 226, 244,241,234,380,100,193,253,329,80,199,374]);

% Call the function
if strcmp(method, 'DFS')
    [path, cost, stepTable] = search_with_table2(graph, startNode, goalNodes);
else
    [path, cost, stepTable] = search_with_table(graph, startNode, goalNodes, heuristicTable, method);
end
...
% ========================================
function [path, cost, stepTable] = search_with_table2(graph, startNode, goalNodes)
...
function [path, cost, stepTable] = search_with_table(graph, startNode, goalNodes,heuristicTable,method)
    ...
    for i = 1:size(neighbors, 1)
        ...
        for j = 1:length(nodes)
            if strcmp(nodes{j}, neighbor)
                ...
                newHCost = heuristicTable(neighbor);
                % Use switch-case to calculate newFCost based on the method
                switch method
                    case 'AStar'
